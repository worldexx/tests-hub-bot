    ...
    "results": [
        {
            ...
            "description": "Спокойный, мягкий, социально точный. В компании — человек-уют.",
        },
        {
            "range": [9, 14],
            "title": "Социальный Хаос-Гений",
            "description": "Появляешься неожиданно, меняешь вайб комнаты, исчезаешь. Странно харизматичный и непредсказуемый.",
        },
        {
            "range": [15, 20],
            "title": "Мемный Коммуникатор",
            "description": "Разговариваешь мемами, думаешь мемами и флиртуешь мемами. Человек-вайб современности.",
        },
        {
            "range": [21, 30],
            "title": "Уверенный Интроверт",
            "description": "Не любишь лишние контакты, но умеешь включать харизму в нужный момент. Точный, редкий тип.",
        },
    ],
}
...
def start_quiz(chat_id: int):
    """Инициализируем сессию и отправляем первый вопрос."""
...
def finish_quiz(chat_id: int):
    ...
    # подбираем результат по диапазону
    result_text = f"Твой результат: {score} баллов"
    for res in QUIZ["results"]:
        low, high = res["range"]
        if low <= score <= high:
            result_text = f"Твой результат: *{res['title']}*\n\n{res['description']}"
            break
...
def handle_quiz_answer(message: types.Message):
    """Обрабатываем ответы только тех, у кого есть активная сессия."""
    ...
    # ищем вариант, который совпадает по тексту
    selected = None
    for opt in question["options"]:
    ...
    if not selected:
        bot.send_message(chat_id, "Выбери один из вариантов на кнопках 👇")
        return

    # обновляем балл и двигаем индекс
